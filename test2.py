...
def launch_python_file():
    ...
    # Access the 'users' collection and check if the folder exists with the given UID
    user_folder_ref = db.collection('users').document(user_uid)
    user_folder = user_folder_ref.get()

    if user_folder.exists:
        # Access the 'User Info' subcollection and retrieve the folder with the given UID
        user_info_collection_ref = user_folder_ref.collection('User Info')
        user_info_folder_ref = user_info_collection_ref.document(user_uid)
        user_info_folder = user_info_folder_ref.get()

        if user_info_folder.exists:
            # Access the document inside the folder
            user_info_doc_ref = user_info_folder_ref.collection(user_uid).document(user_uid)
            ...
            print('User Info folder not found')
